...
class Multitask_DAL_Trainer:
    def __init__(self, model, config):
        """
        Initializes the Multitask_DAL_Trainer with the specified model and configuration.
    ...
    def run_epoch_leave_one_out(self, loader, training):
        """
        Run a single epoch of training or testing for leave-one-out validation.
        ...
        for i, (images, labels, age_groups, genders) in enumerate(loader):
            ...
            wandb.log(metrics)
            if not training:
                if metrics['Testing/id_accuracy'] > accuracy:
    ...
    def train_leave_one_out(self, epochs, train_loader, test_loader):
        ...
        Returns:
            float: Accuracy over the leave-one-out validation.
        """
        accuracy = 0
        for epoch in range(epochs):
