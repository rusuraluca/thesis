...
class CosFaceMarginLoss(nn.Module):
    ...
    def __init__(self, number_of_classes, embedding_size=512, scale=32, margin=0.1):
        """
        Initialize the CosFaceMarginLoss with the specified parameters.
...
class ArcFaceMarginLoss(nn.Module):
    """
    ArcFace Margin Loss introduced in:
    Deng, Jiankang, Guo, Jia, Yang, Jing, Xue, Niannan, Kotsia, Irene, Zafeiriou, Stefanos.
    ArcFace: Additive Angular Margin Loss for Deep Face Recognition.
    IEEE Transactions on Pattern Analysis and Machine Intelligence, 2022.

    This loss function adds an angular margin to the logits by modifying the arc of the angles between the embeddings and the weights.

    Attributes:
        scale (float): The scaling factor applied to the logits.
        margin (float): The margin value added to the angles.
        embedding_weights (nn.Parameter): The learnable weight parameters for the classification layer.
    """
    def __init__(self, number_of_classes, embedding_size=512, scale=32.0, margin=0.1):
        """
        Initializes the ArcFaceMarginLoss with the specified parameters.

        Args:
            number_of_classes (int): The number of classes in the classification task.
            embedding_size (int, optional): The size of the embeddings. Default is 512.
            scale (float, optional): The scaling factor applied to the logits. Default is 32.0.
            margin (float, optional): The margin value added to the angles. Default is 0.1.
        """
    ...
    def forward(self, embeddings, labels):
        """
        Forward pass for the ArcFace margin loss.
        ...
        Returns:
            torch.Tensor: The logits after applying the ArcFace margin loss.
        """
        normalized_embeddings = functional.normalize(embeddings)
        normalized_weights = functional.normalize(self.weights)
        ...
        # gather the logits for the correct class
        theta = logits.gather(1, labels.view(-1, 1)).acos()  # compute the arccosine of the logits for the correct class

        # add the margin to the gathered logits
        target_logits = (theta + self.margin).cos()  # add the margin to the angle and take the cosine

        # scatter the modified logits back to their original positions
        logits.scatter_(1, labels.view(-1, 1), target_logits)  # replace the logits of the correct class with modified logits
